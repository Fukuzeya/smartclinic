...
from billing.api.routes import router
from billing.infrastructure.clinical_event_handler import make_clinical_event_handler
from billing.infrastructure.laboratory_event_handler import make_laboratory_event_handler
from billing.infrastructure.orm import Base  # noqa: F401
from billing.infrastructure.settings import BillingSettings
...
async def _startup_hook(app: FastAPI) -> Callable[[], Awaitable[None]]:
    ...
    lab_handler = make_laboratory_event_handler(session_factory)
    ...
    subscriber.subscribe(Subscription(
        queue_name=settings.laboratory_events_queue,
        handler=lab_handler,
    ))
    await subscriber.start()

    log.info("billing.startup_complete")
