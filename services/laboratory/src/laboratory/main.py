"""Laboratory service entry-point."""
...
from laboratory.api.routes import router
from laboratory.infrastructure.clinical_event_handler import make_clinical_event_handler
from laboratory.infrastructure.orm import Base  # noqa: F401
from laboratory.infrastructure.settings import LaboratorySettings
...
async def _startup_hook(app: FastAPI) -> Callable[[], Awaitable[None]]:
    ...
    # Subscribe to Clinical events so we can receive lab-order-placed notifications
    ...
    log.info("laboratory.startup_complete")
